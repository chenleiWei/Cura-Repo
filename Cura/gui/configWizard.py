...
class InfoBox(wx.Panel):
    def __init__(self, parent):
        ...
        self.sizer.Add(self.extraInfoButton, pos=(0, 2), flag=wx.ALL | wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL,
                       border=5)
...
class SelectPrintHead(InfoPage):
    def __init__(self, parent):
        ...
        self.newPrintHeadRadio.Bind(wx.EVT_RADIOBUTTON, self.OnOptionSelect)
        self.newPrintHeadRadio.SetValue(True)
        self.oldPrintHeadRadio = self.AddRadioButton("No")
        self.oldPrintHeadRadio.Bind(wx.EVT_RADIOBUTTON, self.OnOptionSelect)

    def OnOptionSelect(self, e):
...
class MachineSelectPage(InfoPage):
    ...
    def OnTypeASelect(self, e):
        wx.wizard.WizardPageSimple.Chain(self, self.GetParent().TypeASelectPrintHead)
        ...
        if self.checkupState == 0:
            ...
            if temp[self.checkExtruderNr] > self.startTemp + 40:
                ...
                if self.tempCheckTimeout < 1:
                    ...
                    wx.CallAfter(self.infoBox.SetError, _("Temperature measurement FAILED!"),
                                 'http://wiki.ultimaker.com/Cura:_Temperature_measurement_problems')
                    self.comm.sendCommand('M104 S0 T%d' % (self.checkExtruderNr))
                    self.comm.sendCommand('M104 S0 T%d' % (self.checkExtruderNr))
...
class bedLevelWizardMain(InfoPage):
    ...
    def OnResume(self, e):
        ...
        if self._wizardState == -1:
            ...
            if profile.getMachineSetting('has_heated_bed') == 'True':
                ...
                self.comm.sendCommand('G1 X%d Y%d F%d' % (
                    profile.getMachineSettingFloat('machine_width') / 2.0, profile.getMachineSettingFloat('machine_depth'),
                    feedTravel))
                self.comm.sendCommand('G1 Z0 F%d' % (feedZ))
                self.comm.sendCommand('M400')
                self._wizardState = 3
                ...
                self.comm.sendCommand(
                    'G1 X%d Y%d F%d' % (0, profile.getMachineSettingFloat('machine_depth'), feedTravel))
                self.comm.sendCommand('G1 Z0 F%d' % (feedZ))
                self.comm.sendCommand('M400')
                self._wizardState = 3
            ...
            if profile.getMachineSetting('has_heated_bed') == 'True':
                ...
                self.comm.sendCommand('G1 Z3 F%d' % (feedZ))
                self.comm.sendCommand(
                    'G1 X%d Y%d F%d' % (profile.getMachineSettingFloat('machine_width') - 5.0, 5, feedTravel))
                self.comm.sendCommand('G1 Z0 F%d' % (feedZ))
                self.comm.sendCommand('M400')
                self._wizardState = 7
            else:
                ...
                self.comm.sendCommand('G1 Z3 F%d' % (feedZ))
                self.comm.sendCommand('G1 X%d Y%d F%d' % (profile.getMachineSettingFloat('machine_width') - 5.0,
                                                          profile.getMachineSettingFloat('machine_depth') - 25,
                                                          feedTravel))
                self.comm.sendCommand('G1 Z0 F%d' % (feedZ))
                self.comm.sendCommand('M400')
                self._wizardState = 5
            ...
            self.comm.sendCommand('G1 Z3 F%d' % (feedZ))
            self.comm.sendCommand(
                'G1 X%d Y%d F%d' % (profile.getMachineSettingFloat('machine_width') - 5.0, 20, feedTravel))
            self.comm.sendCommand('G1 Z0 F%d' % (feedZ))
            self.comm.sendCommand('M400')
            self._wizardState = 7
    ...
    def mcTempUpdate(self, temp, bedTemp, targetTemp, bedTargetTemp):
        if self._wizardState == 1:
            self._wizardState = 2
            wx.CallAfter(self.infoBox.SetAttention,
            ...
            if temp[0] < profile.getProfileSettingFloat('print_temperature') - 5:
                wx.CallAfter(self.infoBox.SetInfo, 'Heating up printer: %d/%d' % (
                    temp[0], profile.getProfileSettingFloat('print_temperature')))
            else:
                wx.CallAfter(self.infoBox.SetAttention,
    ...
    def mcStateChange(self, state):
        ...
        if self.comm.isOperational():
            if self._wizardState == 0:
                wx.CallAfter(self.infoBox.SetAttention,
                             'Use the up/down buttons to move the bed and adjust your Z endstop.')
                ...
                wx.CallAfter(self.infoBox.SetInfo,
    ...
    def OnResume(self, e):
        if self._wizardState == 2:
            ...
            self.infoBox.SetAttention(
                'Which horizontal line number lays perfect on top of each other? Front most line is zero.')
            ...
            self.infoBox.SetInfo('Calibration finished. Offsets are: %s %s' % (
                profile.getMachineSettingFloat('extruder_offset_x1'), profile.getMachineSettingFloat('extruder_offset_y1')))
        ...
        if self.comm.isOperational():
            ...
            if not self.comm.isPrinting():
                if self._wizardState == 3:
                    self._wizardState = 4
                    wx.CallAfter(self.infoBox.SetAttention,
                                 'Please measure the distance between the vertical lines in millimeters.')
                    ...
                    self._wizardState = 7
                    wx.CallAfter(self.infoBox.SetAttention,
                                 'Which vertical line number lays perfect on top of each other? Leftmost line is zero.')
