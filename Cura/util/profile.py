...
setting('infill_type',      '3D',  [_('None'),_('2D'),_('3D'),_('Concentric'),_('Gradient grid'),_('Gradient concentric')], 'basic', _('Fill')).setExpertSubCategory(_('Infill')).setLabel(_("Infill Type"), _("Infill is used to add internal geometries.\n Line alternates grid segments per layer. \n Grid generates a grid pattern on each layer. \n Cube generates 3d infill. \n Gradient infills generate a gradient from 100% at the bottom and infill distace on topmost layer"))
setting('fill_distance',               12, float, 'basic',    _('Fill')).setRange(0, 305).setLabel(_("Infill Distance (mm)"), _("This controls the distance between each line of infill"))
...
setting('travel_speed',            200.0, float, 'advanced', _('Speed')).setRange(0.1).setLabel(_("Travel Speed (mm/s)"), _("Speed at which travel moves are done."))
...
setting('fan_full_height',           3, float, 'expert',   _('Cool')).setRange(0).setLabel(_("Fan Full On at Height (mm)"), _("The height at which the fan is turned on completely. For the layers below this the fan speed is scaled linearly with the fan off at layer 0."))
...
setting('2d_infill_type', 		'Automatic',  ['Automatic','Lines','Grid'], 'expert', _('Infill')).setLabel(_("2D Infill Type"), _("Automatic : The engine decides between lines and grid infill.\nLines: The infill is comprised of parallel lines. Infill rotates by 90 degrees on each layer.\nGrid: Infill is a complete grid."))
...
setting('raft_airgap_all', 0, float, 'expert', _('Raft')).setRange(0).setLabel(_("Airgap"), _("Gap between the last layer of the raft the whole print."))
setting('raft_airgap', 0.2, float, 'expert', _('Raft')).setRange(0).setLabel(_("First Layer Airgap"), _("Gap between the last layer of the raft and the first printing layer. A small gap of 0.2mm works wonders on PLA and makes the raft easy to remove. This value is added on top of the 'Airgap' setting."))
...
validators.warningAbove(settingsDictionary['print_temperature'], 300.0, _("Temperatures above 300C could damage your machine, be careful!"))
validators.warningAbove(settingsDictionary['print_temperature2'], 300.0, _("Temperatures above 300C could damage your machine, be careful!"))
validators.warningAbove(settingsDictionary['print_temperature3'], 300.0, _("Temperatures above 300C could damage your machine, be careful!"))
validators.warningAbove(settingsDictionary['print_temperature4'], 300.0, _("Temperatures above 300C could damage your machine, be careful!"))
validators.warningAbove(settingsDictionary['print_temperature5'], 300.0, _("Temperatures above 300C could damage your machine, be careful!"))
...
def OctoPrintAPIRemoveSerial(serial):
	path = os.path.join(getBasePath(), 'octoprint_api_config.ini')
	cp = ConfigParser.ConfigParser()
	try:
		cp.read(path)
...
def setAlterationFileFromFilePath(file):
	cp = configparser.ConfigParser()
	cp.read(file)
